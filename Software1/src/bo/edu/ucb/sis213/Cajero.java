...
public class Cajero {
     ...
     */
    public static boolean validarPIN(Connection connection, int passw) {
        String query = "SELECT alias, saldo FROM usuarios WHERE passw = ?";
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            preparedStatement.setInt(1, passw);
            ResultSet resultSet = preparedStatement.executeQuery();

            if (resultSet.next()) {
                usuarioAlias = resultSet.getString("alias");
     ...
     * @wbp.parser.entryPoint
     */
    public static String obtenerUsuarioIdPorPin(Connection connection, int passwIngresado) throws SQLException {
    String query = "SELECT alias FROM usuarios WHERE passw = ?";
    try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
        preparedStatement.setInt(1, passwIngresado);
        ResultSet resultSet = preparedStatement.executeQuery();
        if (resultSet.next()) {
            return resultSet.getString("alias");
        } else {
            return "";  // No se encontró un usuario con el PIN ingresado
        }
    }
}
